...
export default async function RecruiterDashboardPage() {
  ...
  return <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
    ...
    <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Active candidates" value={String(active)} detail={`${analytics.totals.applications} total applications`}/><MetricCard kind="evidence" label="Need verification" value={String(pendingReviews)} detail="VerifyLoop items awaiting action"/><MetricCard label="Interviews today" value={String(interviewsToday)} detail="Scheduled for today"/><MetricCard kind="decision" label="Decision ready" value={String(readyDecisions)} detail="85%+ decision coverage, still active"/></div>
    ...
    <section className="mt-5 grid gap-3 md:grid-cols-3"><ActionCard href="/recruiter/assessments" title="Assessment Studio" description="Build reusable criterion-linked verification."/><ActionCard href="/recruiter/interviews/availability" title="Interview availability" description="Publish slots for candidate self-scheduling."/><ActionCard href="/recruiter/ai-transparency" title="AI transparency" description="Inspect provider, fallback, latency, and hashes."/></section>
  </main>;
}
