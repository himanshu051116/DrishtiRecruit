...
export function interviewIcs(input: { id: string; title: string; description: string; start: Date; durationMinutes?: number; location?: string; url?: string }) {
  ...
  const lines = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//DrishtiRecruit//Interview Calendar//EN", "CALSCALE:GREGORIAN", "METHOD:PUBLISH", "BEGIN:VEVENT", `UID:${escapeIcs(input.id)}@drishtirecruit`, `DTSTAMP:${utc(new Date())}`, `DTSTART:${utc(input.start)}`, `DTEND:${utc(end)}`, `SUMMARY:${escapeIcs(input.title)}`, `DESCRIPTION:${escapeIcs(input.description)}`];
